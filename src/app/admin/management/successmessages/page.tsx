...
const generateData = (): SuccessMessage[] => {
  return Array.from({ length: 5380 }, (_, i) => {
    const id = i + 1;
...
export default function SuccessMessagesPage() {
  const router = useRouter();
  const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [allData, setAllData] = useState<SuccessMessage[]>([]);
  const [filteredData, setFilteredData] = useState<SuccessMessage[]>([]);
  const [displayData, setDisplayData] = useState<SuccessMessage[]>([]);
  ...
  const columns = [
    {
      ...
      key: 'id',
      width: 60,
      render: (id: number) => <a href={`/admin/management/successmessages/${id}`}>{id}</a>,
    },
    {
      title: 'Message ID',
      dataIndex: 'messageId',
    ...
    },
    {
      ...
      key: 'actions',
      width: 80,
      render: (_: any, record: SuccessMessage) => (
        <Button
          type="text"
          danger
          icon={<DeleteOutlined />}
  ...
  const handleDeleteSingle = (record: SuccessMessage) => {
    Modal.confirm({
  ...
  const handleAdd = () => {
    router.push('/admin/management/successmessages/add');
  };
      ...
      <main className="flex-1 p-8 ml-64 overflow-x-hidden">
        <div className="max-w-[calc(100vw-20rem)]">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Успешные сообщения</h1>
              <div className="text-sm text-gray-500 mt-1">
                ...
                &nbsp;&rsaquo;&nbsp;
                <span className="text-gray-600">Успешные сообщения</span>
              </div>
            </div>
